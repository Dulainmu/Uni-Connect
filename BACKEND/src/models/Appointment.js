...
appointmentSchema.pre('validate', function (next) {
  if (this.startTime && this.endTime) {
    const startTime = convertToMinutes(this.startTime);
    const endTime = convertToMinutes(this.endTime);
...
appointmentSchema.statics.checkTimeConflict = async function (staffId, date, startTime, endTime, excludeId = null) {
  ...
  return conflictingAppointments.some(appointment => {
    const existingStart = convertToMinutes(appointment.startTime);
    const existingEnd = convertToMinutes(appointment.endTime);
